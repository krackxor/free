    ...
    if AI == True: 
        ...
        if user_id:
            try:
                ...
                btn=[
                        [InlineKeyboardButton(text=f"⇱🤷‍♀️ Mengambil tindakan 🗃️⇲", url=f'https://t.me/{temp.U_NAME}')],
                        [InlineKeyboardButton(text=f"🗑 Hapus log ❌", callback_data=f'close_data')],
                    ]
                reply_markup=InlineKeyboardMarkup(btn)
                footer_credit = "🦋<a href='https://t.me/+tYoMgLhGml1lNzQ1'>• MASALAH LAPORAN •</a>══<a href='https://t.me/TMID_CSbot'>• Kontak Admin •</a>🦋"
                lazy_response = response.choices[0].text 
                await client.send_message(LAZY_AI_LOGS, text=f"⚡️⚡️#AI_Query \n\n• Seorang pengguna bernama **{message.from_user.mention}** dengan id pengguna - `{user_id}`. Menanyakan pertanyaan ini kepada saya...\n\n══❚█══Q   U   E   R   Y══█❚══\n\n\n[Q྿.]**{lazy_users_message}**\n\n👇Inilah yang saya tanggapi:\n:-`{lazy_response}`\n\n\n❚═USER ID═❚═• `{user_id}` \n❚═USER Name═❚═• `{message.from_user.mention}` \n\n🗃️" , reply_markup = reply_markup )
